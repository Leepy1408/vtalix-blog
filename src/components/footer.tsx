  ...
  return (
    <footer className="bg-white text-sm text-gray-700">
      <div className="bg-gray-100">
        <div className="container mx-auto px-4 py-4 flex flex-col md:flex-row justify-between items-center text-center md:text-left gap-4">
            ...
            <div className="shrink-0 ml-4">
                 <Image src="https://placehold.co/150x50.png" data-ai-hint="razorpay logo" alt="Payments Powered by Razorpay" width={130} height={40} />
            </div>
        </div>
      </div>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-6">
            <nav className="flex flex-wrap gap-x-6 gap-y-2 justify-center items-center">
               {mainLinks.map((link) => (
                <Link key={link.name} href={link.to} className="hover:text-primary hover:underline font-medium text-gray-800">
                  {link.name}
                </Link>
              ))}
            </nav>
            <div className="flex items-center gap-4">
                {socialIcons.map((social, index) => (
                   <a key={index} target="_blank" rel="noopener noreferrer" href={social.href} className='text-gray-600 hover:text-primary'>{social.icon}</a>
                ))}
            </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mt-6 text-gray-500 gap-4">
